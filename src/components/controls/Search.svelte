...
<div id="search-bar" class="hidden lg:flex transition-all items-center h-10 rounded-full
...
">
    ...
    <input id="search-input-desktop" placeholder="Ctrl K" aria-label={i18n(I18nKey.search)} bind:value={keywordDesktop}
           on:focus={() => search(keywordDesktop, true)}
           class="transition-all pl-10 text-sm bg-transparent outline-0
         h-full w-22 active:w-60 focus:w-60 text-black/60 dark:text-white/60"
    >
</div>
